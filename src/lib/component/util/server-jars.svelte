<script lang="ts">
    ...
    async function getServerJarDownloadUrl(type, version) {
        const latestVersion = await getLatestServerJarVersionIfNotExists(type, version); // Assuming this helper function exists
        const response = await fetch(`/api/server-jars/${type}/${latestVersion}`);
        const data = await response.json();
